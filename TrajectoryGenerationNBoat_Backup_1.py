...
class TrajectoryGenerationNBoat:
	...
	def define_action_reward(self):
		...
		#boat_direction
		theta = math.atan2((self.endPointCol - self.positionCol), (self.endPointRow - self.positionRow))
		...
		return all_actions, only_diagonal
	...
	def create_map(self, qtable, blocks=None):
		...
		while not end_step:
			...
			map.loc[self.positionCol, self.positionRow] = 1
		...
		map.loc[self.endPointCol, self.endPointRow] = 16
	...
	def train(self, learning_rate, discount, random_explore, horizon, blocks):
		all_actions, only_diagonal = self.define_action_reward()
		only_diagonal=False
